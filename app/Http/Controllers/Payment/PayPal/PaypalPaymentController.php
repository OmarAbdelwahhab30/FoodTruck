...
class PaypalPaymentController extends Controller
{
    ...
    public function payWithpaypal(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $transaction_ref = Str::random(6) . '-' . rand(1, 1000);
        ...
        $amount->setCurrency(Helpers::currency_code())->setTotal($number);
        session()->put('transaction_reference', $transaction_ref);
        $transaction = new Transaction();
        $transaction->setAmount($amount)
            ->setItemList($item_list)
            ->setDescription($transaction_ref);
